...
public void drawLine(int x0, int y0, int x1, int y1, Color color) {
    ...
    if ((code0 & code1) == 0) {
        // Ambos puntos están dentro o en el límite del área de recorte
        if (code0 != 0) {
            // Si el punto inicial está fuera del área de recorte, lo movemos al límite
            x0 = adjustX(x0, y0, x1, y1);
            y0 = adjustY(x0, y0, x1, y1);
        }
        if (code1 != 0) {
            // Si el punto final está fuera del área de recorte, lo movemos al límite
            x1 = adjustX(x1, y1, x0, y0);
            y1 = adjustY(x1, y1, x0, y0);
        }
...
private int adjustX(int x, int y, int xOther, int yOther) {
    int xAdjusted = x;
    if (x < clipX0) {
        xAdjusted = clipX0;
    } else if (x > clipX1) {
...
private int adjustY(int x, int y, int xOther, int yOther) {
    int yAdjusted = y;
    if (y < clipY0) {
        yAdjusted = clipY0;
    } else if (y > clipY1) {
